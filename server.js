  ...
  const filas = ordenes.map(o => {
    if (o.IdGrupoSellado != null && gruposFusionables.has(o.IdGrupoSellado)) {
      ...
      if (ancla.Estado === 'Pendiente') {
        accionesGrupo = `<button type="button" class="btn-accion btn-iniciar" onclick="abrirEscaneoRollo(${ancla.IdOrden}, false)">▶ Iniciar</button>`;
    ...
    if (necesitaTomarControl) {
      ...
      acciones += `
        ${o.IdGrupoSellado == null ? `<button type="button" class="btn-accion btn-anadir" onclick="abrirEscaneoRollo(${o.IdOrden}, true)">+ Rollo</button>` : ''}
...
function renderOrdenDetalle(orden, totalBultos, historial, usuario, maquinaCodigo, pausaActiva, avance, proximaCalidad, grupoSellado) {
  ...
  if (orden.Estado === 'Pendiente') {
    ...
    acciones = `
      ${grupoSelladoOtras.length === 0 ? `<button type="button" class="btn-accion btn-anadir" onclick="abrirEscaneoRollo(${orden.IdOrden}, true)">+ Rollo</button>` : ''}
...
function renderGrupoSelladoDetalle(idGrupo, numeroPedido, maquinaNombre, maquinaCodigo, miembros, usuario, historial) {
  ...
  const btnRolloGrupoHTML = miembroActivoAhora ? `
    <div class="orden-cola" style="margin-bottom:16px;">
      ...
      <div class="orden-acciones">
        <button type="button" class="btn-accion btn-anadir" onclick="abrirEscaneoRollo(${miembroActivoAhora.IdOrden}, true)">+ Rollo</button>
      </div>
    </div>` : '';
...
app.get('/selladora/:codigo/orden/:idOrden', requireLogin, async (req, res) => {
  ...
  try {
    ...
    res.send(renderOrdenDetalle(orden, totalBultos, historial, req.session.usuario.nombre, codigo, pausaActiva, avance, proximaCalidad, grupoSellado));
  } catch (err) {
    res.status(500).send(renderErrorSimple(err.message, `/selladora/${codigo}`));
  }
});
